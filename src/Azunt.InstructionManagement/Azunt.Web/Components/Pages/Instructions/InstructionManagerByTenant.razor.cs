...
public partial class InstructionManagerByTenant : ComponentBase
{
    ...
    protected string? TenantConnectionString { get; set; }

    protected string DisplayConnectionString => string.IsNullOrWhiteSpace(TenantConnectionString)
        ? "Default In-Memory repository"
        : TenantConnectionString.StartsWith(InstructionAppDbContextFactory.InMemoryConnectionPrefix, StringComparison.OrdinalIgnoreCase)
            ? TenantConnectionString
            : "SQL Server tenant connection string";
    ...
    private void LoadTenantContext()
    {
        ...
        TenantConnectionString = user.Tenant.ConnectionString;
    ...
    protected async Task DeleteClick()
    {
        if (model.Id != 0)
        {
            await RepositoryReference.DeleteAsync(model.Id, TenantConnectionString);
        }
    ...
    protected async void ToggleClick()
    {
        ...
        await RepositoryReference.UpdateAsync(model, TenantConnectionString);
        IsInlineDialogShow = false;
        model = new Instruction();
        await DisplayData();
    }
    ...
    {
        Nav.NavigateTo("/api/InstructionExport/ExcelByTenant", forceLoad: true);
    }
